...
Пошук ключавых слоў вядзецца ў **загалоўку**, **катэгорыі** (`category`) і **тэгах** кожнага запісу (без уліку рэгістру).
...
## Выдаленне старых запісаў

Запісы старэй за `MAX_AGE_DAYS` дзён аўтаматычна выдаляюцца з выніковага фіда. У логах адлюстроўваецца колькасць выдаленых запісаў:

```
INFO: 🗑 Выдалена старых запісаў (>30d): 47
```

Пры `MAX_AGE_DAYS=0` фільтрацыя па ўзросце адключаецца.

---

## Блакіроўка па чорным спісе

Пошук ключавых слоў адбываецца па **цэлых словах** — частковыя супадзенні ігнаруюцца. Напрыклад, keyword `ІІ` не заблакуе слова `памяшканні`. У логах відаць слова-трыгер:

```
INFO: 🚫 Заблакавана [sponsored]: Buy crypto — best deals this week
INFO: 🚫 Заблакавана [рэклама]: Спецыяльная прапанова ад партнёраў
```
